...
public class PlayerManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Detección rotación
        //if (myRigidbody2D.velocity.x > 0.0f && rotate) EventManager.RotateEvent(gameObject, false);
        //else if (myRigidbody2D.velocity.x < 0.0f && !rotate) EventManager.RotateEvent(gameObject, true);
        if (myRigidbody2D.velocity.x > 0.0f && rotate) RotatePlayer(false);
        else if (myRigidbody2D.velocity.x < 0.0f && !rotate) RotatePlayer(true);
    }
